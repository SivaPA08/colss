...
#pragma omp parallel for reduction(+ : var_sum)
        ...
        // store result for return after GIL reacquired
        // reuse sum variable to carry it out of the block
        sum = std::sqrt(var_sum / n);

    } // MODIFIED: GIL reacquired here
    ...
    return sum; // holds sqrt(var_sum / n)
}
